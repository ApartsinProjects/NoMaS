...
<p>Unsupervised anomaly detection ships without labels, so choosing which detector and hyperparameters to trust is itself an unsupervised problem. Many model-selection methods have been proposed to solve it, and each is reported to improve on the last. But the datasets used to evaluate these methods are contaminated: anomalies that a single feature or a one-line rule already separates, anomalies that are exact copies of normal points, normals that leak between train and test, tasks no detector can solve, base rates so high that the metric degenerates, and comparisons reported without significance. A selection method judged on such data is credited for artifacts, not for skill at picking a good detector.</p>

<p>We introduce ADReal, a benchmark for evaluating unsupervised model selection, built by a single detector-free pipeline and audited until every one of these channels is closed. Detectors are scored on one base-rate-normalized metric, and a selection method is scored by its regret against the oracle-best detector, with paired significance. From 173 constructed tasks, 151 survive filters that remove the trivial and the unsolvable. On this benchmark we evaluate roughly thirty classical and deep detectors under a single consistent preprocessing, and eight selection methods.</p>

<p>The honest protocol overturns the standing picture. Under leakage-free validation on normals only, the agreement-based selectors that lead in transductive evaluation (consensus, model-centrality, HITS) fall below a random pick; only entropy and mass-volume survive. We then introduce SPARC, which grades detectors on label-free synthetic probes built from the normal data. SPARC is the only selector that significantly beats both survivors (regret 0.124 versus 0.176 for entropy and 0.174 for mass-volume, $p<0.001$ against each). The advantage is decisive on time series (0.05 versus 0.15, $p<0.001$) and holds on tabular data. We release the benchmark, the per-dataset detector scores, and the code.</p>
</div>
