...
<p>Anomaly detectors are judged on flawed benchmarks. Normal points leak from training into test sets, some labeled anomalies are exact copies of normal points, some tasks are solved by a single feature, and differences are reported without a significance test. These flaws bias every comparison built on such data, of detectors and of the label-free methods that choose among them.</p>

<p>We introduce ADReal, a benchmark that closes each of these channels. It matches how these methods are used: both picking a detector for a new dataset and evaluating one are done without seeing that dataset's anomalies, so every label-free criterion is computed on held-out normal data alone. After filtering out trivial and unsolvable tasks, 151 datasets remain; on them we evaluate roughly thirty classical and deep detectors and eight selection methods.</p>

<p>Choosing a detector without labels turns out to be nearly a coin flip. The agreement-based selectors that lead under standard evaluation fall below a random pick; only two criteria, entropy and mass-volume, hold up. We then introduce SPARC, which builds synthetic anomalies from the normal data and grades detectors on how well they find them. SPARC is the only method that significantly beats both survivors: regret, the gap to the best available detector, is 0.124 versus 0.176 for entropy and 0.174 for mass-volume, $p<0.001$ against each. The gain is decisive on time series (0.05 versus 0.15, $p<0.001$) and holds on tabular data. We release the benchmark, the per-dataset detector scores, and the code.</p>
</div>
...
1. **ADReal, a dataset and evaluation protocol that makes anomaly-detection comparison reliable.** A single detector-free, reproducible pipeline removes trivial, unsolvable, and mislabeled tasks, closes train/test leakage, caps the base rate, deduplicates anomalies, and scores every method by regret against the oracle-best detector with paired significance. Because both choosing a detector for a new dataset and evaluating one are done without labels, every criterion is computed on held-out normals only, matching how these methods are used; 151 tasks survive from 173 constructed.
2. **An evaluation of a rich set of modern detectors.** We fit roughly thirty classical and deep anomaly detectors under one consistent preprocessing, fixing the per-task oracle-best detector that defines regret.
3. **An evaluation of state-of-the-art unsupervised model-selection methods.** Under the leakage-free protocol, the widely-cited agreement selectors (consensus, model-centrality, HITS) fall below a random pick, and only entropy and mass-volume survive, reversing their apparent lead under transductive evaluation.
