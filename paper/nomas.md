...
<p>Anomaly detection methods are ranked on benchmarks that gather many datasets of normal data and labeled anomalies. These benchmarks are frequently contaminated and their metrics inadequate, so the rankings are unreliable: normals leak from training into test, some labeled anomalies are exact copies of normal points, some tasks are trivially easy while others are unsolvable, and scores ignore how rare the anomalies are or are read at unrealistic anomaly rates, often with no significance test. Such flaws bias every comparison built on the data, of detectors and of the label-free methods that choose among them.</p>

<p>We build ADReal by curating existing benchmarks: we filter out contaminated anomalies and datasets that are too easy, too hard, or leaky, and deduplicate the rest, leaving 151 clean tasks. On them we evaluate an array of popular classical and deep detectors, scored by a metric normalized to each dataset's anomaly rate so results are comparable across datasets.</p>

<p>Anomalies are not representative samples by definition, so the right detector for a dataset must be chosen from its normal data alone, often with no trustworthy anomalies at all. The popular criteria that do this are themselves methods that need a fair benchmark to be judged on, and on ADReal most of them fail: the widely-used approaches that rank detectors by how much they agree with one another do worse than picking at random, and only two criteria, entropy and mass-volume, beat a random pick.</p>

<p>We introduce SPARC, a model-selection method that builds synthetic anomalies from the normal data and grades detectors on how well they find them. It is the only method to significantly beat both survivors: regret, the gap to the best available detector, is 0.124 versus 0.176 for entropy and 0.174 for mass-volume, $p<0.001$ against each. The gain is decisive on time series (0.05 versus 0.15) and holds on tabular data. We release the benchmark, the detector scores, and the code.</p>
</div>
