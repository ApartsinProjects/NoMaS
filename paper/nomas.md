...
<p>New anomaly detection methods introduced in the literature demonstrate their performance on benchmarks: collections of datasets, each with normal data and labeled anomalies, that rank the methods by how well they recover the anomalies. But these benchmarks are often contaminated, and their metrics are weak, so the rankings cannot be trusted. Normal points leak from training into test. Some anomalies are exact copies of normal points. Some tasks are trivially easy and others are impossible. And the usual scores ignore how rare anomalies are, or measure at anomaly rates no real system would face.</p>

<p>We build the ADReal benchmark by curating existing ones. Filtering out contaminated anomalies and datasets that are too easy, too hard, or leaky, and removing duplicates, leaves 151 clean datasets. On these we evaluate a range of popular detectors, classical and deep, scoring each with a metric that accounts for how rare a dataset's anomalies are, so results compare fairly across datasets.</p>

<p>Anomalies are not representative samples by definition, so in practice the right detector for an application-specific dataset must be chosen from its normal data alone, often with no reliable anomalies to check against. The methods that make this choice deserve a fair benchmark of their own. On ADReal, most do no better than guessing: the popular methods that pick whichever detector the others agree with score worse than a random choice, and only two standard selection methods, entropy and mass-volume, beat it.</p>

<p>We introduce SPARC, a selection method that makes its own synthetic anomalies from the normal data and grades detectors by how well they catch them. SPARC is the only method to significantly beat both survivors: its regret, the gap to the best available detector, is 0.124, against 0.176 for entropy and 0.174 for mass-volume, with $p<0.001$ in each case. The gain is large on time series (0.05 versus 0.15) and holds on tabular data. We release the benchmark, the detector scores, and the code.</p>
</div>
