...
<p>Anomalies are not representative samples by definition, so the right detector for a dataset must be chosen from its normal data alone, often with no trustworthy anomalies at all. The popular criteria that do this are methods in their own right, and they need a fair benchmark to be judged on. On ADReal the choice is nearly a coin flip: agreement-based selectors fall below a random pick, and only entropy and mass-volume hold up.</p>
