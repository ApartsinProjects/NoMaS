...
FRACS = [0.3, 0.6, 1.0]
print("=== CONTROLLED combination-weird generator (feature permutation), regret of probe pick (in-dist%) ===")
...
for corp, name, tf in TARGETS:
    ...
    for fr in FRACS:
        syn = gen_combo(Xval, 300, fr); ind = (nn.kneighbors(sc.transform(syn))[0][:, 0] <= scale).mean()
